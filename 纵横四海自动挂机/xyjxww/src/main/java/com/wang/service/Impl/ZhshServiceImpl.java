...
import com.wang.mapper.ZhshMapper;
import com.wang.pojo.Zhsh;
import com.wang.service.ZhshService;
import com.wang.task.AsyncSaveSql;
...
@Service
public class ZhshServiceImpl extends ServiceImpl<ZhshMapper,Zhsh> implements ZhshService{
    ...
    public void attack() {
           ...
           while (stop) {
                ...
                if (home.getBody().contains("攻击")){
                    String find4 = Jsoup.parse(home.getBody()).select("a:matchesOwn(^攻击$)").attr("href");
                    HttpResponse<String> stringHttpResponse = Unirest.get(find4).asString();

                }

               HttpResponse<String> home1 = Unirest.get("/Map/340_264?sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3").asString();
                String find = Jsoup.parse(home1.getBody()).select("a:contains(哥斯拉龙)").attr("href");
                // 进入攻击状态，请求1
                HttpResponse<String> attack = Unirest.get(find).asString();



                String attackOne = Jsoup.parse(attack.getBody()).select("a:matchesOwn(^攻击$)").attr("href");
                // 第一次攻击 请求2
                HttpResponse<String> attack2 = Unirest.get(attackOne).asString();
    ...
    public Zhsh getGame() {
        ...
        if (user.getBody().contains("撤退")){
            String exit = Jsoup.parse(user.getBody()).select("a:contains(撤退)").attr("href");
            HttpResponse<String> use = Unirest.get(exit).asString();
        }

        HttpResponse<String> user1 = Unirest.get("/User/Index/Index?sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3").asString();
