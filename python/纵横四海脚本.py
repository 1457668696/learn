...
while True:
    ...
    r = requests.get('http://sea.pccsh.com/Map/257_517?sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3&_r=3EiLu5GcgqhW',
                     verify=False,
                     headers=headers)
    html = r.text
    ...
    for a in selector.xpath('//a[text()="四瞳灵狐"]'):
        ...
        r = requests.get(f'http://sea.pccsh.com{startUrl}',
                         verify=False,
                         headers=headers)
        print(f"---------------------第{count}攻击-------------------------------")
    ...
    for a1 in selector.xpath('//a[text()="攻击"]'):
        ...
        r1 = requests.get(f'http://sea.pccsh.com{attUrl}',
                          verify=False,
                          headers=headers)
        ...
        for a3 in selector.xpath('//a[text()="攻击"]'):
            ...
            r3 = requests.get(f'http://sea.pccsh.com{attUrl}',
                              verify=False,
                              headers=headers)
            ...
            if len(three) ==1 :
                ...
                for a22 in selector.xpath('//a[text()="攻击"]'):
                    ...
                    r22 = requests.get(f'http://sea.pccsh.com{attUrl}',
                                       verify=False,
                                       headers=headers)
            ...
            if count % 50 == 0:
                # 进入地图
                r6 = requests.get(
                    'http://sea.pccsh.com/Map/Index/MapCity/257_517?sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3',
                    verify=False,
                    headers=headers)
                print("进入地图")
                # 进入铁匠铺
                ...
                for a6 in selector.xpath('//a[text()="铁匠铺"]'):
                    ...
                    r8 = requests.get(f'http://sea.pccsh.com{attUrl}',
                                      verify=False,
                                      headers=headers)
                    print("进入铁匠铺")
                ...
                #             进入修理界面
                r5 = requests.get(
                    'http://sea.pccsh.com/Bag/Equ/RecoverEqu?npc=1502009&sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3',
                    verify=False,
                    headers=headers)
                ...
                for a5 in selector.xpath('//a[contains(text(),"全部修复")]'):
                    ...
                    # 修理成功
                    r5 = requests.get(f'http://sea.pccsh.com{attUrl}',
                                      verify=False,
                                      headers=headers)
                ...
                for a8 in selector.xpath('//a[text()="凤凰山"]'):
                    ...
                    r10 = requests.get(f'http://sea.pccsh.com{attUrl}',
                                       verify=False,
                                       headers=headers)
    if count % 30 == 0:
        r11 = requests.get(f'http://sea.pccsh.com/User/Index/Index?sid=2HsGTdIXRs71FJNFjsImIT320Cf51JF3',
                           verify=False,
                           headers=headers)
        ...
        if len(allex) == 1:
            ...
            with open("../output.txt", "a", encoding="utf-8") as f:
                ...
                print(f"当前等级：{level[0]}")
                print(f"当前经验{nowEx}万")
                print(f"升级还需要经验：{needEx / 10000}万")
